...
class BKMEmpleo private(
  ...
  /**
    ...
    */
  @Since("1.6.0")
  def this() = this(4, 20, 1.0, classOf[BKMEmpleo].getName.##)
...
private object BKMEmpleo extends Serializable {
      ...
      if (divisibleIndices.contains(index)) {
        ...
        if (newClusterChildren.nonEmpty) {
          val selected = newClusterChildren.minBy { child =>
            KMeansEmpleo.fastSquaredDistance(newClusterCenters(child), v)
          }
          (selected, v)
        } else {
          (index, v)
    ...
    def buildSubTree(rawIndex: Long): ClusteringTreeNode = {
      ...
      if (isInternal) {
        ...
        val height = math.sqrt(indexes.map { childIndex =>
          KMeansEmpleo.fastSquaredDistance(center, clusters(childIndex).center)
        }.max)
...
private[clustering] class ClusteringTreeNode private[clustering](
  ...
  private def predictPath(pointWithNorm: VectorWithNorm): List[ClusteringTreeNode] = {
    if (isLeaf) {
      ...
      val selected = children.minBy { child =>
        KMeansEmpleo.fastSquaredDistance(child.centerWithNorm, pointWithNorm)
      }
  ...
  private def predict(pointWithNorm: VectorWithNorm): (Int, Double) = {
    predict(pointWithNorm, KMeansEmpleo.fastSquaredDistance(centerWithNorm, pointWithNorm))
  }
  ...
  private def predict(pointWithNorm: VectorWithNorm, cost: Double): (Int, Double) = {
    if (isLeaf) {
      ...
      val (selectedChild, minCost) = children.map { child =>
        (child, KMeansEmpleo.fastSquaredDistance(child.centerWithNorm, pointWithNorm))
      }.minBy(_._2)
