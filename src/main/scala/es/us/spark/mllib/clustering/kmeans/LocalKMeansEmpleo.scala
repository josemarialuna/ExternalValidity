...
/**
  ...
  */
private[mllib] object LocalKMeansEmpleo extends Logging {
  ...
  def kMeansPlusPlus(
                      seed: Int,
                      points: Array[clustering.VectorWithNorm],
                      weights: Array[Double],
                      k: Int,
                      maxIterations: Int
                    ): Array[clustering.VectorWithNorm] = {
    val rand = new Random(seed)
    val dimensions = points(0).vector.size
    val centers = new Array[clustering.VectorWithNorm](k)

    // Initialize centers by sampling using the k-means++ procedure.
    centers(0) = pickWeighted(rand, points, weights).toDense
    val costArray = points.map(KMeansEmpleo.fastSquaredDistance(_, centers(0)))
    ...
    for (i <- 1 until k) {
      ...
      for (p <- points.indices) {
        costArray(p) = math.min(KMeansEmpleo.fastSquaredDistance(points(p), centers(i)), costArray(p))
      }
    ...
    while (moved && iteration < maxIterations) {
      ...
      while (i < points.length) {
        val p = points(i)
        val index = KMeansEmpleo.findClosest(centers, p)._1
