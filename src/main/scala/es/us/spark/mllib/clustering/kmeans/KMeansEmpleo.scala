...
class KMeansEmpleo private(
  ...
  /**
    ...
    */
  @Since("0.8.0")
  def this() = this(2, 20, KMeansEmpleo.K_MEANS_PARALLEL, 2, 1e-4, Utils.random.nextLong())
  ...
  def setInitializationMode(initializationMode: String): this.type = {
    KMeansEmpleo.validateInitMode(initializationMode)
  ...
  private def runAlgorithm(
                            data: RDD[clustering.VectorWithNorm],
                            instr: Option[Instrumentation[NewKMeans]]): KMeansModel = {
    ...
    val centers = initialModel match {
      ...
      case None =>
        if (initializationMode == KMeansEmpleo.RANDOM) {
          initRandom(data)
        } else {
    ...
    while (iteration < maxIterations && !converged) {
      ...
      val totalContribs = data.mapPartitions { points =>
        ...
        points.foreach { point =>
          val (bestCenter, cost) = KMeansEmpleo.findClosest(thisCenters, point)
          costAccum.add(cost)
      ...
      totalContribs.foreach { case (j, (sum, count)) =>
        ...
        if (converged && KMeansEmpleo.fastSquaredDistance(newCenter, centers(j)) > epsilon * epsilon) {
          converged = false
        }
        centers(j) = newCenter
      }
  ...
  /**
    ...
    */
  private def initRandom(data: RDD[clustering.VectorWithNorm]): Array[clustering.VectorWithNorm] = {
  ...
  /**
    ...
    */
  private[clustering] def initKMeansParallel(data: RDD[clustering.VectorWithNorm]): Array[clustering.VectorWithNorm] = {
    ...
    val centers = ArrayBuffer[clustering.VectorWithNorm]()
    ...
    while (step < initializationSteps) {
      ...
      costs = data.zip(preCosts).map { case (point, cost) =>
        math.min(KMeansEmpleo.pointCost(bcNewCenters.value, point), cost)
    ...
    if (distinctCenters.size <= k) {
      ...
      val countMap = data.map(KMeansEmpleo.findClosest(bcCenters.value, _)._1).countByValue()

      bcCenters.destroy(blocking = false)

      val myWeights = distinctCenters.indices.map(countMap.getOrElse(_, 0L).toDouble).toArray
      LocalKMeansEmpleo.kMeansPlusPlus(0, distinctCenters.toArray, myWeights, k, 30)
    }
  }
}
...
object KMeansEmpleo {
  ...
  private[mllib] def findClosest(
                                  centers: TraversableOnce[clustering.VectorWithNorm],
                                  point: clustering.VectorWithNorm): (Int, Double) = {
  ...
  private[mllib] def pointCost(
                                centers: TraversableOnce[clustering.VectorWithNorm],
                                point: clustering.VectorWithNorm): Double =
  ...
  private[clustering] def fastSquaredDistance(
                                               v1: clustering.VectorWithNorm,
                                               v2: clustering.VectorWithNorm): Double = {
    val sqdist = sqdistEmpleo(v1.vector, v2.vector)

    //println(sqdist)
    sqdist
  }

  def sqdistEmpleo(v1: Vector, v2: Vector): Double = {
    ...
    while (kv < sz) {
      ...
      squaredDistance += Math.abs(score)
      kv += 1
    }
    //val meanSquaredDistance = squaredDistance / sz
  ...
  private[spark] def validateInitMode(initMode: String): Boolean = {
    initMode match {
      case KMeansEmpleo.RANDOM => true
      case KMeansEmpleo.K_MEANS_PARALLEL => true
      case _ => false
    }
  }
}
...
class VectorWithNorm(val vector: Vector, val norm: Double) extends Serializable {
  ...
  def toDense: clustering.VectorWithNorm = new VectorWithNorm(Vectors.dense(vector.toArray), norm)
}
